...
is
   ...
   is
      ...
      --  Check inequality of logical resource names.
      procedure Check_Inequality (Left, Right : DOM.Core.Node);
         ...
         begin
            Component_Name := To_Unbounded_String
              (DOM.Core.Elements.Get_Attribute
                 (Elem => Comp_Node,
                  Name => "name"));
            if DOM.Core.Nodes.Length (List => Resources) > 1 then
               Mulog.Log (Msg => "Checking uniqueness of"
                          & DOM.Core.Nodes.Length (List => Resources)'Img
                          & " memory resource names in component '"
                          & To_String (Component_Name) & "'");
               Mucfgcheck.Compare_All (Nodes      => Resources,
                                       Comparator => Check_Inequality'Access);
            end if;
         end;
      end loop;
   end Component_Memory_Resource_Name_Uniqueness;
